...
<body>
	<c:set var = "colors" value = "red, green, blue, gray, pink, orange"/>
	<h4>${colors}</h4>
	<c:forTokens var = 'a' items = "${colors}" delims=",">
		<div class = "box" style = "background-color: ${a.trim()}">${a.trim()}</div>
	</c:forTokens>
	<hr>
	...
	currency: <fmt:formatNumber value = "${money}" type = "currency" currencySymbol = "$"/><br><!-- , 화폐단위 추가  -->
	
	<fmt:formatNumber value = "${num1}" pattern = "0.00"/><br>
	<fmt:formatNumber value = "${num2}" pattern = "0.00"/><br>
	<fmt:formatNumber value = "${money}" pattern = "#,##0"/><br><!-- 3자리마다 ,찍기 -->
</body>
</html>